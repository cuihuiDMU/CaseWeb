...
<div class="wrapper" id="contenter">
 ...
 <div class="nav"></div>
   <div class="listmenu">
    <div class="listtt" id="newsALL"><span>新闻动态 </span></div>
     <div class="lists">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr><td class="list" id="newsChina">国内动态</td></tr>
          <tr><td class="list" id="newsOther">国外动态</td></tr>
          </table>
     </div>
   </div><!--end listmenu-->
   <div class="listcol">     
     <div class="name" id="newspagetitle">国内外动态</div>   
     <div class="listbox">
      <div class="entry" id="entry">
        <table id="newspagelist" class="nplist">
         </table>
      </div>
   </div>  
      <div class="container">
...
var options = {  
  ...
  onPageClicked: function (e, originalEvent, type, page) { 
  	buildTable(page,PAGESIZE,"newsALL"); //默认每页最多10条
  }  
}  

//生成表格
function buildTable(pageNumber,pageSize,country) {
   var reqParams = {'pageNumber':pageNumber,'pageSize':pageSize,"newscountry":country};//请求数据
   $(function () {   
   	  $.ajax({
   	        type:"POST",
   	        url:"<%=request.getContextPath()%>/newsPageServlet",
   	        data:reqParams,
   	        dataType:"json",
   ...
   // $("#newspagelist").empty();//清空表格内容
    if (dataList.length > 0 ) {
  	  ...
  	  $("#newspagelist").html(str);  
	          ...
	          $("#newspagelist").html('<tr><td><center>查询无数据</center></td></tr>');
	      }
...
$(".list,.listtt").click(function(){
  ...
  var newstitle=$(this).html();
  $("#newspagetitle").html(newstitle);
//生成底部分页栏
  $("#pageTab").bootstrapPaginator(options);     
	buildTable(1,PAGESIZE,newscountry);//默认空白查全部
}); 
</script>
</html>